...
export const openTicketPdfInBrowser = (ticketId) => {
  ...
  window.open(directUrl, '_blank') || (window.location.href = directUrl);
};
...
export const downloadQrImage = async (ticketId) => {
  if (!ticketId) return;
  const qrUrl = `${API_BASE_URL}/api/ticket/qr/${ticketId}`;
  const filename = `CloudBus_QR_BP${String(ticketId).padStart(6, '0')}.png`;
  try {
    ...
    console.warn('Blob QR download fallback:', e);
    const link = document.createElement('a');
    link.href = qrUrl;
    link.download = filename;
    link.target = '_blank';
...
export const generateClientPassPdf = async (ticket) => {
  ...
  try {
    ...
    doc.setFillColor(isPending ? 254 : 243, isPending ? 243 : 244, isPending ? 199 : 246);
    doc.rect(15, 59, 180, 12, 'F');
    doc.setDrawColor(isPending ? 245 : 209, isPending ? 158 : 213, isPending ? 11 : 219);
    ...
    if (isPending) {
      ...
      doc.text(`STATUS: PENDING APPROVAL`, 105, 66.5, { align: 'center' });
    } else {
